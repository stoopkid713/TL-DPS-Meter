        ...
        function renderPartyHistoryTab() {
            ...
            const activeId = partyState.active_encounter_id;
            // STEP 1 (no-boss suppression): mirror the encounter-switcher filter (see
            // renderEncounterSwitcher) — show only boss-resolved encounters + the live/active
            // one, so the no-boss "Recording…" churn (35 of 52 in the ZUY5 run) stays out of the
            // Encounters list, not just the switcher dropdown.
            const list = (partyState.encounters || [])
                .filter((e) => e.boss || (e.encounter_id === activeId && !e.ended))
        ...
        const RELEASES_PAGE = 'https://github.com/stoopkid713/STOOP/releases/latest';
